...
def is_valid_result(token):
    """
    Cek apakah token adalah angka result togel yang valid.
    Exclude: tahun (1900-2099), angka tidak masuk akal untuk togel.
    """
    if token.lower() == "xxxx":
        return False  # handle terpisah
    try:
        n = int(token)
        # Exclude tahun & angka yang bukan range togel wajar
        if 1900 <= n <= 2099:
            return False
        return True
    except ValueError:
...
def scrape_pasaran(path):
    ...
    try:
        ...
        # === STRATEGI 1: Cari di elemen tabel <td> yang isinya tepat 4 digit ===
        td_results = []
        for td in soup.find_all("td"):
            txt = td.get_text(strip=True)
            if re.match(r"^\d{4}$", txt) and is_valid_result(txt):
                td_results.append(txt)
            elif txt.lower() == "xxxx":
                td_results.append(txt)

        # === STRATEGI 2: Cari di elemen dengan class/id hasil (angkanet-style) ===
        for el in soup.find_all(class_=re.compile(r"result|angka|keluaran|pengeluaran", re.I)):
            txt = el.get_text(strip=True)
            m = re.search(r"\b(\d{4})\b", txt)
            if m and is_valid_result(m.group(1)):
                td_results.append(m.group(1))

        # === STRATEGI 3: Fallback scan semua teks ===
        text = soup.get_text(separator=" ")
        all_tokens = re.findall(r"\b(\d{4}|xxxx)\b", text, re.IGNORECASE)

        # Filter: valid result saja (bukan tahun dll)
        valid_tokens = [t for t in all_tokens if t.lower() == "xxxx" or is_valid_result(t)]

        # Gabungkan kandidat: prioritaskan dari tabel, fallback ke semua teks
        candidates = td_results if td_results else valid_tokens

        if not candidates:
            print(f"  [KOSONG] Tidak ada result valid - {url}")
            return None, False

        # Cek pending (xxxx terakhir)
        last = candidates[-1].lower()
        is_pending = (last == "xxxx")

        # Ambil angka valid terakhir (bukan xxxx)
        valid_only = [t for t in candidates if t.lower() != "xxxx"]
        if not valid_only:
            return None, is_pending

        result = valid_only[-1]
        print(f"  [DEBUG] Candidates sample (last 5): {candidates[-5:]}")
